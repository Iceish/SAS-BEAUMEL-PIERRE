...
@section('main')
    ...
    <form action="{{route('dashboard.profile.update')}}" method="post" autocomplete="off">
        ...
        <div class="field">
            ...
            <input type="text" id="name" name="name" value="{{ $user->name }}" placeholder="{{ __('custom/words.data.input.text.name.placeholder') }}" autocomplete="off" />
        </div>
        ...
        <input type="submit" class="btn" value="{{ __('custom/words.data.input.submit.default.label') }}">
    </form>
@endsection
