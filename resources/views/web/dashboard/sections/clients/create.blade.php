...
@section('main')
    <x-utils.backBtn/>
    <h2>{{ ucfirst(__('custom/words.data.crud.creating', ['item' => trans_choice('custom/words.client', 1)])) }}</h2>
    ...
    <form id="edit" action="{{ route("dashboard.clients.store") }}" method="post">
        ...
        <div class="field">
            ...
            <input type="text" id="name" name="name" placeholder="{{ __('custom/words.data.input.text.fullname.placeholder') }}" value="{{ old('name') }}"/>
        </div>
